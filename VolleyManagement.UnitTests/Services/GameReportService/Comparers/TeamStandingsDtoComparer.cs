namespace VolleyManagement.UnitTests.Services.GameReportService
{
    ...
    internal class TeamStandingsDtoComparer : IComparer<TeamStandingsDto>
    {
        public int Compare(TeamStandingsDto x, TeamStandingsDto y)
        {
            ...
            Assert.AreEqual(x.TeamId, y.TeamId, "TeamId do not match");

            Assert.AreEqual(x.Points, y.Points, "Points do not match");
            Assert.IsTrue(AreNullableFloatsEqual(x.SetsRatio, y.SetsRatio), $"SetsRatio do not match. Actual:<{x.SetsRatio}>, Expected:<{y.SetsRatio}>");
            Assert.IsTrue(AreNullableFloatsEqual(x.BallsRatio, y.BallsRatio), $"SetsRatio do not match. Actual:<{x.BallsRatio}>, Expected:<{y.BallsRatio}>");
