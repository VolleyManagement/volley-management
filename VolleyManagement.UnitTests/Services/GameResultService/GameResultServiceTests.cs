namespace VolleyManagement.UnitTests.Services.GameResultService
{
    ...
    public class GameResultServiceTests
    {
        ...
        private readonly Mock<IQuery<GameResultRetrievable, FindByIdCriteria>> _getByIdQueryMock
            = new Mock<IQuery<GameResultRetrievable, FindByIdCriteria>>();
        ...
        public void TestInit()
        {
            ...
            _kernel.Bind<IQuery<GameResultRetrievable, FindByIdCriteria>>().ToConstant(_getByIdQueryMock.Object);
        ...
        {
            // Arrange
            var newGameResult = new GameResultStorableBuilder().Build();
        ...
        {
            // Arrange
            var newGameResult = null as GameResultStorable;
            var sut = _kernel.Get<GameResultService>();
        ...
        {
            // Arrange
            var newGameResult = new GameResultStorableBuilder().WithTheSameTeams().Build();
            var sut = _kernel.Get<GameResultService>();
        ...
        {
            // Arrange
            var newGameResult = new GameResultStorableBuilder().WithInvalidSetsScore().Build();
            var sut = _kernel.Get<GameResultService>();
        ...
        {
            // Arrange
            var newGameResult = new GameResultStorableBuilder().WithSetsScoreNoMatchSetScores().Build();
            var sut = _kernel.Get<GameResultService>();
        ...
        {
            // Arrange
            var newGameResult = new GameResultStorableBuilder().WithInvalidRequiredSetScores().Build();
            var sut = _kernel.Get<GameResultService>();
        ...
        {
            // Arrange
            var newGameResult = new GameResultStorableBuilder().WithInvalidOptionalSetScores().Build();
            var sut = _kernel.Get<GameResultService>();
        ...
        {
            // Arrange
            var newGameResult = new GameResultStorableBuilder().WithPreviousOptionalSetUnplayed().Build();
            var sut = _kernel.Get<GameResultService>();
        ...
        {
            // Arrange
            var newGameResult = new GameResultStorableBuilder().WithSetScoresUnordered().Build();
            var sut = _kernel.Get<GameResultService>();
        ...
        {
            // Arrange
            var expected = new GameResultRetrievableBuilder().WithId(GAME_RESULT_ID).Build();
            var sut = _kernel.Get<GameResultService>();
            ...
            // Assert
            TestHelper.AreEqual(expected, actual, new GameResultRetrievableComparer());
        }
        ...
        public void GetTournamentResults_GameResultsRequsted_GameResultsReturned()
        {
            ...
            // Assert
            CollectionAssert.AreEqual(expected, actual, new GameResultRetrievableComparer());
        }
        ...
        {
            // Arrange
            var gameResult = new GameResultStorableBuilder().Build();
            var sut = _kernel.Get<GameResultService>();
        ...
        private bool AreGameResultsEqual(GameResultStorable x, GameResultStorable y)
        {
            return new GameResultStorableComparer().Compare(x, y) == 0;
        }

        private void SetupGet(GameResultRetrievable gameResult)
        {
            _getByIdQueryMock.Setup(m => m.Execute(It.Is<FindByIdCriteria>(c => c.Id == gameResult.Id))).Returns(gameResult);
        }

        private void SetupGetTournamentResults(int tournamentId, List<GameResultRetrievable> gameResults)
        {
        ...
        private void SetupEditMissingEntityException(GameResultStorable gameResult)
        {
            _gameResultRepositoryMock.Setup(m =>
                m.Update(It.Is<GameResultStorable>(grs => AreGameResultsEqual(grs, gameResult))))
                .Throws(new ConcurrencyException());
        }

        private void VerifyCreateGameResult(GameResultStorable gameResult, Times times)
        {
            _gameResultRepositoryMock.Verify(
                m => m.Add(It.Is<GameResultStorable>(grs => AreGameResultsEqual(grs, gameResult))), times);
            _unitOfWorkMock.Verify(m => m.Commit(), times);
        }

        private void VerifyEditGameResult(GameResultStorable gameResult, Times times)
        {
            _gameResultRepositoryMock.Verify(
                m => m.Update(It.Is<GameResultStorable>(grs => AreGameResultsEqual(grs, gameResult))), times);
            _unitOfWorkMock.Verify(m => m.Commit(), times);
        }

        private void VerifyEditGameResult(GameResultStorable gameResult, Times repositoryTimes, Times unitOfWorkTimes)
        {
            _gameResultRepositoryMock.Verify(
                m => m.Update(It.Is<GameResultStorable>(grs => AreGameResultsEqual(grs, gameResult))), repositoryTimes);
